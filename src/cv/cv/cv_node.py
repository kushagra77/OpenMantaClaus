...
class CVNode(Node):
    ...
    def _capture_frame(self) -> None:
        ...
        if self._playback:
            ...
            if not self._cap.isOpened():
                return
            # CAMERA THROTTLE / HEARTBEAT LOGIC / smart exposure preprocess
            if not self._active:
                now_ns = self.get_clock().now().nanoseconds
                if now_ns - self._last_inactive_process_ns < 500_000_000:
                    return
        ...
        if not self._active:
            if self._debug_image:
                ...
                image_msg.header.frame_id = "manta_camera"
