...
/**
 * @class EKFSLAM
 * @brief ROS 2 Node for AUV EKF SLAM with a fixed 10-feature map.
 */
...
public:
     ...
     * @param y_noise Measurement noise for the constraint along y axis.
     * */
    void apply_relative_constraint(int id_a, int id_b, double dx, double dy, double y_noise=1e-6);
    
    /**
     * @brief gets imu msg and updates odometry
     * 
     * @param msg 
     */
...
private:
    ...
    static constexpr int state_size_ = 3 + (2 * feature_count_) + 3; // 3 for robot pose, 2 per feature, 3 for past
    ...
    double bearing_std_dev_ = 0.01; // Standard deviation for bearing noise
    ...
    // New flare tuning parameters (populated from params)
    ...
    /**
     * @brief constrains the flares to be between the gate and flag and within +-7m in y axis
     * 
     */
